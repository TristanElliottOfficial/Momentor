...
@section('title', 'Login with the credetials you provided when registering.')

@section('content')

  <!-- Login Authentication View Start -->
    <div id="login" class="flex justify-center items-center bg-gray-200 h-screen">
        ...
        <div class="w-2/5 xl:w-1/5 sm:w-3/5 xs:w-4/5">
          ...
          <!-- Logo Container Start -->
            <div class="flex px-2 items center py-6">
              ...
              <div class="pl-3">
                ...
                <p class="font-light text-gray-500">{{ __('Please fill out the form below to login') }}</p>
              </div>
            </div>
          <!-- Logo Container End -->
          ...
          <!-- Authentication Form Start -->
            <div class="px-4 py-4 bg-white shadow-lg rounded">
              <form id="login_form" action="{{ route('login') }}" method="post" role="form">
                @csrf
                <div class="py-2">
                  ...
                  <input type="email" id="email" name="email" class="block px-4 py-2 w-full rounded bg-gray-100 font-light border focus:outline-none @error('password') border-red-500 @enderror" placeholder="{{ __('Enter an Email Address') }}" />
                  @error ('email')
          ...
          <!-- Auth Links Start -->
            <div id="auth_link" class="flex justify-between mt-4">
              <a href="{{ route('password.request') }}" class="block py-3 font-light text-gray-500">{{ __('Forgot your password?') }}</a>
              <a href="{{ route('register') }}" class="block py-3 font-light text-gray-500">{{ __('Dont have an Account?') }}</a>
            </div>
          <!-- Auth Links End -->
